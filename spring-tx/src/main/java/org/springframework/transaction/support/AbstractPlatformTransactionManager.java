...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// Use defaults if no transaction definition given. 当前事务为null，则使用默认的事务
		...
		// 判断当前线程是否存在事务，如果存在就按照嵌套事务的逻辑来处理
		...
		// 如果当前事务信息的超时事件 < 默认的，就抛出 InvalidTimeoutException
		if (def.getTimeout() < TransactionDefinition.TIMEOUT_DEFAULT) {
			throw new InvalidTimeoutException("Invalid transaction timeout", def.getTimeout());
		}

		// No existing transaction found -> check propagation behavior to find out how to proceed.
		// 当前线程不存在事务，并且行为是 PROPAGATION_MANDATORY 就抛出 IllegalTransactionStateException
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		// 当前线程已经存在的事务的传播行为是下列之一的话就会先被挂起，然后把他的属性给保存到SuspendedResourcesHolder中
		else if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRED ||
				def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW ||
				def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			//先挂起，并且封装已经存在的事务的属性，为了方便新事务执行完成后，再恢复这个事务的状态
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		//如果当前线程已经存在事务，把这个事务的属性信息封装成SuspendedResourcesHolder 返回
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				// 将当前线程事务的激活状态设置为false
				TransactionSynchronizationManager.setActualTransactionActive(false);
				// 将获取到的事务属性封装成 SuspendedResourcesHolder 并返回
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		// 如果状态已经是完成，抛出 IllegalTransactionStateException
	...
	public final void rollback(TransactionStatus status) throws TransactionException {
		// 如果事务已经提交完成，执行回滚会抛出 IllegalTransactionStateException
		if (status.isCompleted()) {
			throw new IllegalTransactionStateException(
					"Transaction is already completed - do not call commit or rollback more than once per transaction");
		}
				...
				// 如果是独立事务
					...
					if (status.hasTransaction()) {
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							...
							//如果当前事务不是独立事务，做个标记，等所有事务执行完成后统一回滚
	 ...
	 *
	 * 看 DataSourceTransactionManager 的逻辑
	 */
