...
public abstract class AopUtils {
	 ...
	 *
	 * 方法大体逻辑：
	 * 		1.拿到Pointcut的ClassFilter然后去判断类是否匹配
	 * 		2.拿到Pointcut的MethodMatcher然后去判断方法是否匹配
	 * 			2.1 MethodMatcher.TRUE 表示所有方法都匹配 直接返回true即可
	 * 			2.2 拿到当前类涉及的所有方法，只要有一个匹配就返回true
	 *
	 * 分析事务的话，PointCut的实现类是 TransactionAttributeSourcePointcut
	 */
	public static boolean canApply(Pointcut pc, Class<?> targetClass, boolean hasIntroductions) {
		...
		if (!Proxy.isProxyClass(targetClass)) {
			//如果当前类不是代理类，就把当前类加入classes
			classes.add(ClassUtils.getUserClass(targetClass));
		}
		//得到当前类实现的所有接口
		classes.addAll(ClassUtils.getAllInterfacesForClassAsSet(targetClass));

		//遍历所有当前类涉及的所有方法，只要有一个匹配就返回true
	...
	public static boolean canApply(Advisor advisor, Class<?> targetClass, boolean hasIntroductions) {
		...
		//我们一般的PointCut是来到这个else if的（属于per-class）
		// 其中，事务对应的PointCut是 TransactionAttributeSourcePointcut
	 ...
	 *
	 * 大体逻辑：
	 * 	1.把符合条件的candidateAdvisors处理并放入eligibleAdvisors
	 * 	2.把candidateAdvisors中没有处理过的处理并加入eligibleAdvisors
	 *
	 * canApply()：是通过PointCut接口的matches()去查看方法是否匹配（即是否是增强的方法）
	 */
