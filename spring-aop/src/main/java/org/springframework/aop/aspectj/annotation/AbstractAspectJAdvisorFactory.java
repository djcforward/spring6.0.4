...
public abstract class AbstractAspectJAdvisorFactory implements AspectJAdvisorFactory {
	...
	public boolean isAspect(Class<?> clazz) {
		//如果当前类标有@Aspect && 类中的字段不存在以ajc$开头的
	...
	public void validate(Class<?> aspectClass) throws AopConfigException {
		...
		//当前类的父类标注了@Aspect && 当前类的父类不是抽象类就抛 AopConfigException
		...
		//当前类为标注@Aspect就抛NotAnAtAspectException
		if (!ajType.isAspect()) {
			throw new NotAnAtAspectException(aspectClass);
		}
		//当前类关联的kind信息是PerClauseKind.PERCFLOW || PerClauseKind.PERCFLOWBELOW 都会抛AopConfigException
	...
	protected static AspectJAnnotation<?> findAspectJAnnotationOnMethod(Method method) {
		//遍历AspectJ的注解，判断当前方法是不是被其中一个标注，是的话就返回标注的那个注解
