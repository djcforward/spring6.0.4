...
public class AdvisedSupport extends ProxyConfig implements Advised {
	 ...
	 * 拦截器链，在Bean实例化后的代理中就会把所有advisor放到这个成员变量中
	 */
	@SuppressWarnings("serial")
	...
	/**
	 * Add all the given advisors to this proxy configuration.
	 * 把所有的advisors放入代理配置中去，也就是advisors成员变量
	 * @param advisors the advisors to register
	 */
	public void addAdvisors(Collection<Advisor> advisors) {
		...
		if (!CollectionUtils.isEmpty(advisors)) {
			for (Advisor advisor : advisors) {
				...
				Assert.notNull(advisor, "Advisor must not be null");
				// 把所有的advisors放入代理配置中去，也就是advisors成员变量，以后调用方法就会去执行相应织入逻辑了
	 ...
	 * 获取拦截器链
	 *
	 * 从缓存中获取拦截器链，如果缓存中没有就用advisorChainFactory获取拦截器链（advisors），然后在放到缓存中去
	 * 最后返回拦截器链
	 *
	...
	public List<Object> getInterceptorsAndDynamicInterceptionAdvice(Method method, @Nullable Class<?> targetClass) {
		...
		if (cached == null) {
			//通过advisorChainFactory获取拦截器链（advisors）
