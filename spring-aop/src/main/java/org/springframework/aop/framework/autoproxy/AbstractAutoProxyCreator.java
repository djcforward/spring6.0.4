...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	 ...
	 * @return 代理对象（需要代理的话） || null（不需要代理或者已经处理过）
	 *
	 * 这个方法在Bean实例化之前就会调用，
	 * 具体调用是在AbstractAutowireCapableBeanFactory.applyBeanPostProcessorsBeforeInstantiation()
	 */
	@Override
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		// 这个beanName长度为0 || targetSourcedBeans不包括beanName
		if (!StringUtils.hasLength(beanName) || !this.targetSourcedBeans.contains(beanName)) {
			if (this.advisedBeans.containsKey(cacheKey)) {//表示已经处理过了
				return null;
			}

			//没有处理过，如果是基础设施类 || 需要跳过（看当前类是不是切面类，是的话就跳过） 就不处理当前Bean
			if (isInfrastructureClass(beanClass) || shouldSkip(beanClass, beanName)) {
				//就将当前key加入advisedBeans表示已经处理过
	...
	protected Object getCacheKey(Class<?> beanClass, @Nullable String beanName) {
		if (StringUtils.hasLength(beanName)) {//beanName不为空
			//检测是不是FactoryBean 如果是的话还要拼接一个前缀，不是就直接返回beanName
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		//如果不需要对当前Bean增强，直接返回当前Bean
		if (Boolean.FALSE.equals(this.advisedBeans.get(cacheKey))) {
			return bean;
		}
		//如果当前类是基础设施类或者已经设置了跳过自动代理，就直接返回
		if (isInfrastructureClass(bean.getClass()) || shouldSkip(bean.getClass(), beanName)) {
			this.advisedBeans.put(cacheKey, Boolean.FALSE);
			return bean;
		}

		// Create proxy if we have advice. 当前Bean需要增强
		//获取需要AOP增强的方法，对这些需要增强的方法创建Proxy,事务也是在这里处理的
		Object[] specificInterceptors = getAdvicesAndAdvisorsForBean(bean.getClass(), beanName, null);
		if (specificInterceptors != DO_NOT_PROXY) {//表示当前Bean需要增强
			this.advisedBeans.put(cacheKey, Boolean.TRUE);
			//创建代理
	...
	protected boolean isInfrastructureClass(Class<?> beanClass) {
		//如果当前Bean是Advice、Pointcut、Advisor、AopInfrastructureBean其中一种 就会返回true
	 ...
	 *
	 * 如果是代理不是事务的话，看AspectJAwareAdvisorAutoProxyCreator的实现方法
	 */
	...
	/**
	 * 通过proxyFactory.getProxy()真正去创建代理对象
	 * @param beanClass
	 * @param beanName
	...
	private Object buildProxy(Class<?> beanClass, @Nullable String beanName,
		...
		/** 初始化Bean之后，创建代理的核心代码------begin*/
		//构造拦截器链
		Advisor[] advisors = buildAdvisors(beanName, specificInterceptors);
		//为proxyFactory设置拦截器链，目标对象，生成之后是否还能产生变动等
	 ...
	 *
	 * 这里我们看AbstractAdvisorAutoProxyCreator的实现方法
	 */
	@Nullable
