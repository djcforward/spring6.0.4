...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	...
	/**
	 * 执行目标方法，但是在执行目标方法之前，会拿到排序后的拦截器（增强方法），执行增强方法
	 * 事务执行目标方法也会来到这里
	 * @return
	 * @throws Throwable
	 */
	...
	public Object proceed() throws Throwable {
		...
		/**
		 * 如果 currentInterceptorIndex 指向 interceptorsAndDynamicMethodMatchers.size - 1
		 * 表示拦截器已经执行完了，直接执行目标方法，即invokeJoinpoint()
		 * 注意：如果interceptorsAndDynamicMethodMatchers的长度为0，因为currentInterceptorIndex初始值是-1.
		 * 		所以理所当然会执行 invokeJoinpoint()，假设是事务的话，因为没有这一类拦截器（@Before，@After等等）
		 * 		所以长度为0，会直接调用 invokeJoinpoint()
		 */
		if (this.currentInterceptorIndex == this.interceptorsAndDynamicMethodMatchers.size() - 1) {
			return invokeJoinpoint();//执行目标方法
		}

		//拿到当前位置的interceptorsAndDynamicMethodMatcher(请注意，interceptorsAndDynamicMethodMatchers是有顺序的)
		Object interceptorOrInterceptionAdvice =
				this.interceptorsAndDynamicMethodMatchers.get(++this.currentInterceptorIndex);
		//判断当前拦截器是不是InterceptorAndDynamicMethodMatcher，是的话就执行下面的if
		...
		//我们的拦截器不是InterceptorAndDynamicMethodMatcher类型，所以会来到这个else里面，调用对应拦截器的逻辑
		//对于spring事务来说，这里的实现类是 TransactionInterceptor
		else {
