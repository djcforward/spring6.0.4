...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	protected boolean isAutowireCandidate(
			...
			throws NoSuchBeanDefinitionException {
		/**@Autowired  核心代码 ---------------begin*/
		String bdName = BeanFactoryUtils.transformedBeanName(beanName);
		if (containsBeanDefinition(bdName)) {
			/**@Autowired 重点看这一行代码即可**********/
	 ...
	 * to be injected into other beans which declare a dependency of matching type.
	 *
	 * 确定指定的 bean 定义是否有资格作为自动装配候选者，以注入到声明匹配类型依赖项的其他 bean 中。
	 *
	 * @param beanName the name of the bean definition to check
...
//		创建一个副本以允许 init 方法依次注册新的 bean 定义。 虽然这可能不是常规工厂引导程序的一部分，但它确实可以正常工作。
		/**实例化非懒加载的单例Bean ---------begin*/
		...
		// Trigger post-initialization callback for all applicable beans... 触发所有符合条件的 bean 的初始化后回调...
		for (String beanName : beanNames) {
	...
	// 获取@value，@Autowired、@Inject所对应的内容
	@Override
	@Nullable
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			if (value != null) {
				if (value instanceof String strValue) {//如果获取到的数据是String，就来到这个if
					//@Value 看这个方法
			...
			if (matchingBeans.size() > 1) {
				//确定是哪个Bean注入到当前属性，determineAutowireCandidate()返回这个BeanName
...
//			isAutowireCandidate(candidate, descriptor)：判断是否是要注入的Bean
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		//解析被@Primary标注的类或者方法，最终返回被@Primary标注的BeanName
	 ...
	 * @param candidates a Map of candidate names and candidate instances <beanName,bean实例>
	 ...
	 *
	 * 大体逻辑：
	 * 		1.创建 primaryBeanName 用于接收最终结果
	 * 		2.遍历Map得到每一个beanName和bean实例
	 * 		3.通过beanName和bean实例判断这个是Bean是不是被@Primary标注
	 * 			3.1 不是的话就遍历下一个Bean
	 * 			3.2 是的话判断primaryBeanName是否为空
	 * 				3.2.1 不为空，
	 * 					判断是否同时存在candidateLocal和primaryLocal（用于判断@Primary是否标注了两个类型相同的Bean）
	 * 						如果存在就抛异常
	 * 						不存在直接把candidateBeanName赋值给primaryBeanName
	 * 				3.2.2 为空，直接把candidateBeanName赋值给primaryBeanName
	 */
	@Nullable
	protected String determinePrimaryCandidate(Map<String, Object> candidates, Class<?> requiredType) {
		String primaryBeanName = null;//用于接收最终结果
		for (Map.Entry<String, Object> entry : candidates.entrySet()) {
			String candidateBeanName = entry.getKey();
			Object beanInstance = entry.getValue();
			//通过beanName和bean实例判断这个是Bean是不是被@Primary标注，不是的话就看下一个Bean
