...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/** Names of beans that are currently in creation. 记录当前线程正在创建的Bean*/
	 ...
	 * not for actual use 是否获得实例进行类型检查，不用于实际使用
	 *
	 * @return an instance of the bean
	 * @throws BeansException if the bean could not be created
	 *
	 * 方法大体流程：
	 * 1. transformedBeanName(name):FactoryBean的名称是带&的，用于获取Factory本身，这里需要转换成不带&的
	 * 2.从缓存中获取Bean对象（一二三级缓存，涉及循环依赖问题）
	 * 		2.1 根据获取到的对象再去获取真正想要的对象，因为这里可能获取的是FactoryBean（然后再去拿对象），也可能是我们想要的对象
	 * 3.如果缓存中没有就去创建Bean对象
	 * 		3.0 校验有没有多例Bean的循环依赖
	 * 		3.1 查看有没有父类BeanFactory。有的话就用父类去创建Bean对象。
	 * 		3.2 没有的话就自己创建
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			...
			//获取真正需要的Bean（比如说FactoryBean里的Bean）
		...
		/** 3 ****缓存没有，去创建Bean******/
			...
			try {
				...
				/**解析 @Value 并且检查有没有构造器的循环依赖---------end*/

				/** 使用prototypesCurrentlyInCreation记录当前线程正在创建的Bean，用于多例Bean的循环依赖判断 ----begin */
				else if (mbd.isPrototype()) {//如果是多例Bean，会调用对应的createBean(beanName, mbd, args);
					...
					try {
						...
						//把已经创建好的Bean从prototypesCurrentlyInCreation移除
						afterPrototypeCreation(beanName);
					}
					beanInstance = getObjectForBeanInstance(prototypeInstance, name, beanName, mbd);
				}
				/** 使用prototypesCurrentlyInCreation记录当前线程正在创建的Bean，用于多例Bean的循环依赖判断 ----end */
	...
	public String resolveEmbeddedValue(@Nullable String value) {
		...
		for (StringValueResolver resolver : this.embeddedValueResolvers) {
			result = resolver.resolveStringValue(result);//核心方法
			if (result == null) {
				return null;
			}
		}
	...
	protected void beforePrototypeCreation(String beanName) {
		Object curVal = this.prototypesCurrentlyInCreation.get();
		if (curVal == null) {
			//此时当前线程没有正在创建的bean，直接把这个beanName放进去就可以了，此时prototypesCurrentlyInCreation存的是String
			this.prototypesCurrentlyInCreation.set(beanName);
		}
		//创建第二个Bean时会进来（因为此时还是prototypesCurrentlyInCreation存的还是String）
		else if (curVal instanceof String strValue) {
			//此时当前线程存在正在创建的bean，创建一个set集合，把strValue beanName 加入到set集合中
			// 因为是set集合，所以天然保证了无重复 只会同时创建一个Bean
			Set<String> beanNameSet = new HashSet<>(2);//如果strValue beanName不重复，只有两个所以这里容量为2就可以
			beanNameSet.add(strValue);
			beanNameSet.add(beanName);
			this.prototypesCurrentlyInCreation.set(beanNameSet);
		}
		else {//prototypesCurrentlyInCreation存的已经是set集合了，把当前Bean放进去就可以了。
	 ...
	 * @see #isPrototypeCurrentlyInCreation
	 *
	 * 从prototypesCurrentlyInCreation中 移除 已经被成功创建的Bean
	 */
	@SuppressWarnings("unchecked")
	protected void afterPrototypeCreation(String beanName) {
		Object curVal = this.prototypesCurrentlyInCreation.get();
		if (curVal instanceof String) {//只有一个bean在创建
			this.prototypesCurrentlyInCreation.remove();
		}
		else if (curVal instanceof Set<?> beanNameSet) {//有多个bean在同时创建
			beanNameSet.remove(beanName);
			if (beanNameSet.isEmpty()) {//可能set中只有一个元素，所以需要清空prototypesCurrentlyInCreation
				this.prototypesCurrentlyInCreation.remove();
			}
		}
	}
	 ...
	 *
	 * 移除给定bean名称的singleton实例（如果有的话），但前提是它没有被用于类型检查之外的其他目的。
	 */
	 ...
	 * 获取给定 bean 实例的对象，如果是 FactoryBean，则可以是 本身 或 其创建的对象。
	 * @param beanInstance the shared bean instance
	 * @param name the name that may include factory dereference prefix 包括了 & 的name
	 * @param beanName the canonical bean name 不包括 & 的name
	 * @param mbd the merged bean definition
	 * @return the object to expose for the bean
	 *
	 * 方法流程：
	 *  1.获取FactoryBean 判断需要的是不是FactoryBean（通过&来判断），如果是的话，进入if做校验，校验通过 返回FactoryBean
	 *  2.获取普通的Bean 判断是否是普通的Bean，如果是的话 直接返回
	 *  3.获取FactoryBean里的Bean
	 */
	protected Object getObjectForBeanInstance(
		...
		if (object == null) {//缓存没有
			...
			// 从工厂返回 bean 实例。 如果是单例，则缓存从 FactoryBean 获得的对象。
			if (mbd == null && containsBeanDefinition(beanName)) {
				mbd = getMergedLocalBeanDefinition(beanName);
			}
			boolean synthetic = (mbd != null && mbd.isSynthetic());
			//从FactoryBean中获取真正需要的对象
	 ...
	 *
	 * 将给定的 bean 添加到该工厂中的一次性 bean 列表中，
	 * 注册其 DisposableBean 接口和/或在工厂关闭时调用的给定销毁方法（如果适用）。 仅适用于单例。
	 * @param beanName the name of the bean
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {//前提是 一定不是多例
			if (mbd.isSingleton()) {//针对于单例
