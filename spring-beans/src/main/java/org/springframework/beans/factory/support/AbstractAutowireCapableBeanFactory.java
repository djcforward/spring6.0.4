...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 *  未完成的 FactoryBean 实例的缓存：<beanName,BeanWrapper>
	 * */
	...
	/**
	 * 方法的大体逻辑：
	 * 		遍历每一个后置处理器，把当前实例到他们的前置逻辑去处理然后得到经过处理的Bean实例
	 * 		如果当前后置处理器返回null就不会遍历后面的处理器了，直接返回刚才传进来的实例
	 *
	 * 		最终返回的是是经过处理的Bean实例
	 *
	 * 	处理@PostConstruct的逻辑在 InitDestroyAnnotationBeanPostProcessor.postProcessBeforeInitialization()中
	 */
	@Override
	...
	/**
	 * 执行所有BeanPostProcessor.postProcessAfterInitialization() 只要有一个执行的结果数据为null
	 * 就直接返回结果
	 *
	 * 注意：AnnotationAwareAspectJAutoProxyCreator也是一个BeanPostProcessor，
	 * 		所以会调用他的postProcessAfterInitialization()
	 * 		最终实现是由父类AbstractAutoProxyCreator实现的
	 */
	@Override
	 ...
	 * 主要是做一些准备工作，并没有真正创建Bean，会通过调用doCreateBean()去创建Bean并返回
	 * @see #doCreateBean
	 */
	@Override
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			/** 通过特殊的后置处理器，看看是不是要提前生成代理，如果需要的话就会生成代理 不走常规的实例化过程了 */
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (mbd.isSingleton()) {
			/** 0.如果是单例，则去未初始化的的factoryBeanInstanceCache拿BeanWrapper */
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					/** 2. 获取@Value、@Autowired、@PostConstruct、@predestroy、@Resource等标注的字段和方法，
					 * 		然后封装到InjectionMetadata对象中，最后将InjectionMetadata对象存入
		...
		if (earlySingletonExposure) {
			...
			/**3.为bean指定三级缓存的并添加进map中
				将bean和对应的ObjectFactory放到 singletonFactories 里，这里可以理解成常说的把Bean放到三级缓存中
				其实只是三级缓存有了生产对应Bean的能力而已，这里表示 singletonFactory的getObject()的
				方法实现其实就是getEarlyBeanReference() */
			addSingletonFactory(beanName, () -> getEarlyBeanReference(beanName, mbd, bean));
		}

		// Initialize the bean instance.
		Object exposedObject = bean;// bean是真正的实例
		try {
		...
		if (earlySingletonExposure) {
			...
			// 也可能是 null （一级缓存没有 也没经历过三级缓存）如果是null 下面这个if就进不去 那返回的就是原生bean
			// 注意： earlySingletonReference 既可能是原生对象 也可能是代理对象
			Object earlySingletonReference = getSingleton(beanName, false);
			if (earlySingletonReference != null) {
				/**
				 * exposedObject：是经过处理后的对象，处理后会放进缓存，可能是原生对象 也可能是代理对象
				 * 如果是原生对象，这个if才成立
				 */
		...
		// Register bean as disposable. 将 bean 注册为一次性的。就是设置了销毁逻辑
		try {
	 ...
	 *
	 * 获取一个“快捷方式”单例 FactoryBean 实例以用于 {@code getObjectType()} 调用，
	 * 无需完全初始化 FactoryBean。
	 * @param beanName the name of the bean
	 ...
	 *
	 * 请牢记：这一步只是获取Bean中标注了某些注解的字段
	 */
	protected void applyMergedBeanDefinitionPostProcessors(RootBeanDefinition mbd, Class<?> beanType, String beanName) {
		// AutowiredAnnotationBeanPostProcessor 来处理上面@Value、@Autowired、@Inject
		// CommonAnnotationBeanPostProcessor 用于处理@Resource
	 ...
	 *
	 * 使用特殊的后置处理器，生成代理对象，这里的实现类是AbstractAutoProxyCreator
	 *
	 *
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		/** 为@Value、@Autowired、@Inject、@Resource标记的字段赋值，--begin----- */
		if (hasInstantiationAwareBeanPostProcessors()) {
			if (pvs == null) {
				pvs = mbd.getPropertyValues();
			}
			// AutowiredAnnotationBeanPostProcessor 用于处理@Value、@Autowired、@Inject
			// CommonAnnotationBeanPostProcessor 用于处理@Resource
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
		...
		/** 为@Value、@Autowired、@Inject、@Resource标记的字段赋值，end----- */
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		/** 5.0 查看是否实现某些Aware接口 实现了的话就为相应的东西赋值*/
		invokeAwareMethods(beanName, bean);

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			/** 5.1 通过后置处理器的前置逻辑去处理某些逻辑，比如调用@Postonstruct的方法*/
			wrappedBean = applyBeanPostProcessorsBeforeInitialization(wrappedBean, beanName);
		}

		try {
			/**
			 * 5.2 调用重写的InitializingBean.afterPropertiesSet()
			 * 5.3自定义的init-method
			 */
