...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				/**调用注册到工厂的工厂后置处理器的逻辑；
				 * 涉及到的注解有
				 * @Configuaration @ComponentScan @Component @Bean @Import @PropertySource
				 * @Scope
				 * 注册一些符合条件的BeanDefinition*/
	 ...
	 *
	 * 注册并实例化BeanFactoryProcessor，然后处理后置逻辑（就是处理一些注解）
	 * <p>Must be called before singleton instantiation. 在Bean实例化之前必须执行完
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		/**实例化BeanFactoryProcessor，并处理后置逻辑，主要是将一些BeanDefinition其注册到BeanFactory*/
	 ...
	 *
	 * 实例化剩余的所有Bean
	 * 主要就是通过beanFactory.preInstantiateSingletons()去初始化的
	 */
