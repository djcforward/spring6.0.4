...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry registry) {
			...
			/**------分析@ComponentScan、@Component、@Bean、@Import @PropertySource----------begin------------------**/
			/**
			 * 1.根据顺序实例化 BeanDefinitionRegistryPostProcessor对象（BeanFactoryPostProcessor）
			 * 		顺序是（1）implement PriorityOrdered.（2）implement Ordered.（3）其他未实现这两个接口的实现类
			 * 2.都最终调用了 invokeBeanDefinitionRegistryPostProcessors()方法
			 *
			 */
			...
			for (String ppName : postProcessorNames) {
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					//实例化 工厂后置处理器 并且装进currentRegistryProcessors中
			...
			/**--------分析@ComponentScan、@Component、@Bean @Import @PropertySource----------end------------------**/
	...
	private static void invokeBeanDefinitionRegistryPostProcessors(
		...
		for (BeanDefinitionRegistryPostProcessor postProcessor : postProcessors) {
			...
			/**
			BeanFactoryProcessor有很多，当前我们分析的是@Configuration
			所以是ConfigurationClassPostProcessor

			准备配置类，为了能在运行时为 Bean 请求提供服务 并且用 CGLIB 增强的子类替换原生bean。
			 */
