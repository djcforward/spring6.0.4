 ...
 *
 * 当存在多个类型匹配的时候，除了@Autowired和@Qualifier配合来注入某个Bean以外，
 * 还可以通过@Primary指定优先级，标注了@Primary的Bean会被优先注入
 *
 * 涉及的流程：
 * 		1.容器启动阶段，注册配置类的时候扫描配置类里标记了@Primary的Bean，设置到相应Bean信息中
 * 			AnnotatedBeanDefinitionReader.doRegisterBean()
 * 		2.容器启动阶段的BeanFactoryProcessor后置处理阶段
 * 			AbstractApplicationContext.refresh().invokeBeanFactoryPostProcessors(beanFactory);
 * 			最终会到AnnotationConfigUtils.processCommonDefinitionAnnotations()
 * 		3.实例化阶段的填充属性阶段
 * 			AbstractAutowireCapableBeanFactory.doCreateBean().populateBean()
 * 			最终调用DefaultListableBeanFactory.determineAutowireCandidate()
 *
 */
