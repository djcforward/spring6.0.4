 ...
 *
 * 涉及的流程：（较为复杂，建议结合时序图来看）
 *		1.容器的启动化阶段，BeanFactoryProcessor调用后值逻辑注册Bean信息
 *			AbstractApplicationContext.refresh().invokeBeanFactoryPostProcessors()
 *			最终调用ConfigurationClassBeanDefinitionReader.loadBeanDefinitionsForBeanMethod()
 *		2.实例化阶段，调用自定义初始化逻辑(initMethod指定的方法)
 *			AbstractApplicationContext.refresh().finishBeanFactoryInitialization();
 *			最终调用AbstractAutowireCapableBeanFactory.invokeInitMethods()调用自定义初始化方法
 *		3.销毁阶段，容器关闭的时候销毁Bean，最终调用自定义销毁逻辑(destroyMethod指定的方法)
 *			AbstractApplicationContext.close()
 *			最终调用DisposableBeanAdapter.destroy().invokeCustomDestroyMethod()
 *		4.getBean()是如何执行的？
 *			AbstractApplicationContext.getBean()
 *			AbstractBeanFactory.doGetBean()  （比较重要，下面的逻辑就是在这里调用的，这里主要是看缓存里有没有Bean）
 *			最终调用AbstractAutowireCapableBeanFactory.doCreateBean()
 *
 *
 */
...
public @interface Bean {
	 ...
	 * @since 5.1
	 *
	 * 是否支持自动按照类型注入到其他的Bean中（会影响@Autowired，因为他是按照类型注入的，不会影响@Resource）
	 */
