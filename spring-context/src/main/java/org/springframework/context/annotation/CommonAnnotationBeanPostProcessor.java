...
public class CommonAnnotationBeanPostProcessor extends InitDestroyAnnotationBeanPostProcessor
	...
	/**此时还是创建Bean的第二步，构建InjectionMetadata并加入缓存*/
	@Override
	 ...
	 * 填充@Resource标记的字段
	 *
	 * 1.找到@Resource标记的字段，封装成InjectionMetadata
	 * 2.通过inject()给对应的Bean赋值
	...
	/**
	 * 查看缓存有没有metadata，有就直接返回，没有就通过buildResourceMetadata()来构建然后加入缓存
	 * @param beanName
	 * @param clazz
	 * @param pvs
	 * @return
	...
	/**
	 * 1.获取当前类的所有字段，解析@Resource标记的字段，如果当前字段时静态的就直接抛异常
	 * 2.将解析的所有ResourceElement放入InjectionMetadata返回
	 * @param clazz
	 * @return
	 */
