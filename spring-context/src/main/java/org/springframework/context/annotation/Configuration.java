 ...
 * 			最终调用AnnotatedBeanDefinitionReader.doRegisterBean()
 * 		2.容器启动化阶段，BeanFactoryProcessor后置逻辑处理Bean信息
 * 			AbstractApplicationContext.refresh().invokeBeanFactoryPostProcessors(beanFactory);
 * 			最终调用PostProcessorRegistrationDelegate.invokeBeanDefinitionRegistryPostProcessors()
 */
...
public @interface Configuration {
	 ...
	 * @since 6.0
	 *
	 * true表示标记了@Bean的方法必须唯一，这样BeanID就是唯一的，就不会出现多个configuration
	 *   标记了@Bean的方法名相同时，后读取的把前面读取的覆盖
	 *
	 * false表示标记了@Bean的方法不唯一，Bean可能会被覆盖
	 */
