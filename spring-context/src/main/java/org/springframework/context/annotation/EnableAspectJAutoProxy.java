 ...
 *
 * 涉及的流程：
 * 		1.容器的启动阶段，BeanFactoryPostProcessor 将AspectJAutoProxyRegistrar放入
 * 		  <ImportBeanDefinitionRegistrar,AnnotationMetadata> importBeanDefinitionRegistrars 这个map中
 *				ConfigurationClassPostProcessor.processConfigBeanDefinitions()
 *				最终来到ConfigurationClassParser.processImports()
 *		2.容器的启动阶段，BeanFactoryPostProcessor 通过AspectJAutoProxyRegistrar的自定义逻辑加载Bean信息
 *				ConfigurationClassPostProcessor.processConfigBeanDefinitions()
 *					reader.loadBeanDefinitions(configClasses);
 *				最终来到ConfigurationClassBeanDefinitionReader.loadBeanDefinitionsFromRegistrars()
 *		3.Bean实例化阶段
...
public @interface EnableAspectJAutoProxy {
	 ...
	 * @since 4.3.1
	 *  是否对外暴露代理对象 如果暴露，可以通过AopContext进行访问
	 */
