 ...
 *
 * @description 指定配置文件并去加载，将配置文件的内容放到Spring的环境变量中，
 * 				以后可以获取到配置项的内容
 * 1.只能标注在类上
 * 2.通过指定配置文件位置来加载文件内容
 * 3.可以加载properties,xml,yml
 *
 * 涉及的流程：
 * 		容器的启动化阶段，BeanactoryProcessor后置处理阶段，加载指定的配置文件，形成资源
 * 			AbstractApplicationContext.refresh().invokeBeanFactoryPostProcessors()
 * 			最终调用PropertySourceRegistry.processPropertySource()
 *
 */
...
public @interface PropertySource {
	 ...
	 *
	 * 加载的资源的名字，如果为空，会根据加载的配置文件自动生成一个名字
	 */
	String name() default "";
	 ...
	 * @since 4.0
	 * 当配置文件未找到时，是否忽略这个错误，默认时false，不会忽略，没找到就会好错
	 */
