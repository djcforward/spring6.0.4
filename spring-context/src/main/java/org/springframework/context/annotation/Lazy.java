 ...
 *
 *
 * 懒加载，为了优化性能，对于一些使用不频繁的Bean，等我们使用到了 我们才去加载他
 *
 * 涉及的流程：
 * 		AnnotationConfigApplicationContext.register(componentClasses); 创建IOC容器时注册配置类的Bean信息的lazyInit属性
 *
 * 		AnnotationConfigApplicationContext.refresh().invokeBeanFactoryPostProcessors();
 * 			调用BeanFactoryProcessor的后置处理逻辑，处理@Import()引入的符合条件的类，然后给这些Bean信息设置lazyInit属性
 * 			处理普通的标记了@Bean的方法，
 * 		AnnotationConfigApplicationContext.refresh().finishBeanFactoryInitialization(beanFactory);
 * 			实例化Bean，
 *
 *
 */
