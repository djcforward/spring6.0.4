 ...
 *
 * 使用场景：
 * 		用于隔离开发、测试、生产环境，如果IOC容器存在多个类型相同的Bean，但是这些Bean是用于不同的环境的。
 * 		此时可以使用@Profile标识这个Bean是属于哪个环境的，这样当app处于某个环境时，才会注入这个Bean
 *
 *
 * 注意：
 * 		1.@Profile不仅可以标注在方法上，也可以标注在类上，如果是配置类，只有在相应的环境下，该配置类的所有配置才会生效
 * 		2.可以发现@Profile上面标记了@Conditional(ProfileCondition.class)，说明他其实底层还是依靠的@Conditional
 * 			并且自定义的匹配逻辑是由 ProfileCondition 实现的
 *
 * 涉及的流程：
 * 		1.容器启动阶段，BeanFactoryProcessor后置处理逻辑
 * 				AbstractApplicationContext.refresh().invokeBeanFactoryPostProcessors()
 * 				ConfigurationClassBeanDefinitionReader.loadBeanDefinitionsForBeanMethod()
 *				最终调用conditionEvaluator.shouldSkip()
 *
 *
 */
