...
public abstract class ConfigurationClassUtils {
	 ...
	 *
	 * 检查内部类的元数据 （或在配置/组件类中声明的嵌套组件类）。
	 *
	 * @param metadata the metadata of the annotated class
	 * @return {@code true} if the given class is to be registered for
	 * configuration class processing; {@code false} otherwise 如果给定的类会被当成配置类处理就返回true
	 */
	static boolean isConfigurationCandidate(AnnotationMetadata metadata) {
		// Do not consider an interface or an annotation... 不考虑接口或注释..... 如果是当前内部类是接口或注释直接返回false
		if (metadata.isInterface()) {
			return false;
		}

		// Any of the typical annotations found? 找到任何存在的注释？
		//如果是当前内部类标记了@Component，@ComponentScan，@Import，@ImportResource四个之一，就返回true，会被当成配置类处理
		...
		// 如果是当前内部类没任何标记，但是他的内部方法标记了@Bean的方法也会返回true，当作配置类处理
