...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	 ...
	 *
	 *
	 * 在当前类构造器中会调用此方法，注册默认的includeFilters过滤规则
	 * （1）标记了@Component的类 能匹配（@Repository,@Service,@Controller都标记了@Component的）
	 * （2）JSR-250 的 @ManagedBean 能匹配
	 * （3）JSR-330 的 @Named 能撇批
	 */
	...
	/**
	 * Scan the class path for candidate components. 过滤得到想要的BeanDefinition
	 * @param basePackage the package to check for annotated classes
	 * @return a corresponding Set of autodetected bean definitions 相应的一组自动检测的 bean 定义
	 */
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {
		if (this.componentsIndex != null && indexSupportsIncludeFilters()) {
			...
			return scanCandidateComponents(basePackage);//过滤得到想要的BeanDefinition
		}
	}
	...
	/**
	 * 过滤得到想要的BeanDefinition,通过isCandidateComponent()判断当前注解是不是想要扫描的注解
	 *
	 * @param basePackage
	 ...
	 *
	 *判断当前类是否符合过滤器的规则，先判断excludeFilters再判断includeFilters
	 *
	 * 大体逻辑：
	 * 		1.遍历每一个excludeFilter，判断当前类是否与excludeFilters匹配，匹配上了说明被排除了，不能注册
	 * 		2.遍历每一个includeFilter，判断当前类是否与includeFilters匹配，匹配上了检查是否需要跳过
	 * 			2.2 通过	conditionEvaluator.shouldSkip()查看当前Bean需不需要跳过，也就是去匹配@Conditional的规则
	 *
	 * 注意：includeFilters是初始化的时候默认注册了三个过滤器，也就是制定了三个过滤规则
	 * 		具体看registerDefaultFilters()方法
	 */
