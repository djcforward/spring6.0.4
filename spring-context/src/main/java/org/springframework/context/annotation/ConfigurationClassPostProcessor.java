...
public class ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor,
	 ...
	 * 从注册表中的配置类(@Configuration)找到更多的 bean 定义（比如@commpoent @Bean之类的）。
	 */
	@Override
	public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
		...
		//解析@Configuration的类，得到符合过滤规则的BeanDefinition（@Commponent @Bean之类的），并注册
	 ...
	 * @param registry 其实就是一个BeanFactory
	 */
	public void processConfigBeanDefinitions(BeanDefinitionRegistry registry) {
		...
		// Sort by previously determined @Order value, if applicable 按照@Order给配置类排序
		...
		/**重点逻辑************通过 parser 解析@Configuration class*****begin****/
		...
		do {
			...
			/**解析BeanDefinitionHolder，经过这个方法会得到一些BeanDefinition（包括AOP）
			 * 比如，@Bean @Component 标注的方法或类，加载@PropertySource指定的文件
			 * 		@@Import 引入的类
			 * 注意：@Bean 标注的方法 会被封装成BeanMethod放在对应的ConfigurationClass.beanMethods里
			 * 		此时还不是BeanDefinition，在下面才会注册BeanDefinition
			 * */
			...
			/**加载BeanDefinition 比如BeanMethod所对应的Bean信息，给他们再设置一些属性--- begin*/
		...
		/**加载BeanDefinition 比如BeanMethod所对应的Bean信息，给他们再设置一些属性 --- end*/
		/**重点逻辑************通过 parser 解析@Configuration class*****end****/
